...
for (INTER_FOLDER in INTER_FOLDERS) {
  ...
  for (dilfactorvalue in c(0.05, 0.1)) {
    ...
    # Calcular la diferencia entre los valores de éxito
    df_summary_filtered_i <- df_summary_filtered_i %>%
      left_join(df_summary_filtered, by = c("nicheN", "group_number", "dilfactor", "nichedist", "richness")) %>%
      mutate(difference = mean_extinction.x - mean_extinction.y)
    
    p <- ggplot(df_summary_filtered_i, aes(x = group_number, y = difference, color = as.factor(Interaction_value), group = Interaction_value)) +  # next
      geom_line(linewidth = .3) +
      geom_point(size = 1) +
      labs(title = paste(INTER_FOLDER, dilfactorvalue),
           x = "Número de grupo",
           y = "Diferencia en la tasa de extinción (%)",
      ...
      coord_cartesian(ylim = c(-100, 100)) +  # Ajusta los límites del eje y según sea necesario
    ...
    ggsave(
      filename = paste0(out_folder,"/0g_extinction_DIFF_per_group_", INTER_FOLDER, "_", dilfactorvalue, ".png"),
      plot = p,
