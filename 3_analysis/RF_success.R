...
for (threshold in c(0.5, 0.9)) {
  ...
  csv <- read.csv(my_file) %>%
    select(-final_size, -transfer, -filename, -sample) 
  ...
  # my data -----------------------------------------------------------------
  # flexplot((richness) ~ size, data = csv, )
  ...
  test1 <- test_glm(c("size", "dilfactor", "shannon", "richness", "log(dilfactor)"))
  ### Some models are terrible and some others are good:
  
  ### when not including size, all models are very bad (5000 deviance)
  ###       --> success ~ dilfactor:shannon + dilfactor+shannon+richness
  ...
  # <chr>                                                                            <dbl>
  #   1 success ~ size:dilfactor + size+dilfactor+shannon+richness                    328.
  # 2 success ~ size:dilfactor + size+dilfactor+shannon                             328.
  # 3 success ~ size:dilfactor + size+dilfactor+richness                                338.
  # 4 success ~ size:dilfactor + size+dilfactor                                         343.
  # 5 success ~ dilfactor:shannon + size+dilfactor+shannon+richness             357.
  # 6 success ~ dilfactor:shannon + size+dilfactor+shannon                      358.
  # 7 success ~ shannon:richness + size+dilfactor+shannon+richness              361.
  # 8 success ~ size:shannon + size+dilfactor+shannon+richness                  368.
  # 9 success ~ size:shannon + size+dilfactor+shannon                           368.
  # 10 success ~ dilfactor:richness + size+dilfactor+shannon+richness                368.
  # 11 success ~ size:richness + size+dilfactor+shannon+richness                     372.
  # 12 success ~ size+dilfactor+shannon+richness                                     374.
  # 13 success ~ size+dilfactor+shannon                                              375.
  ...
  # 1 success ~ size:log(dilfactor) + size+dilfactor+shannon+richness+log(dilfactor)                   181.
  # 2 success ~ size:log(dilfactor) + size+dilfactor+shannon+log(dilfactor)                            181.
  
  # 3 success ~ size:log(dilfactor) + size+shannon+richness+log(dilfactor)                             182.
  # 4 success ~ size:log(dilfactor) + size+shannon+log(dilfactor)                                      182.
  ...
  # original : "success ~ size:log(dilfactor) + size+shannon+log(dilfactor)"
  
  summary(glm("success ~ size:log(dilfactor) + shannon+log(dilfactor)", data = csv, family = my_family))
  # removing the independent "size" variable increases the deviance to 477.41
  
  summary(glm("success ~ size:log(dilfactor) + size:shannon + log(dilfactor)", data = csv, family = my_family))
  # 484.53
  
  summary(glm("success ~ size:log(dilfactor) + shannon + size + shannon:log(dilfactor)", data = csv, family = my_family))
  # 314.29
  
  summary(glm("success ~ size:log(dilfactor) + size + shannon:log(dilfactor)", data = csv, family = my_family))
  # removing shannon on its own is hurtful to the model as well. 700
  
  summary(glm("success ~ size:log(dilfactor):shannon  + size + log(dilfactor)", data = csv, family = my_family))
  # 220.59
  
  ### Change diversity metric again
  myformula <- "success ~ size:log(dilfactor) +  evenness  + size  + log(dilfactor)"
  ...
  summary(glm("success ~ size:log(dilfactor) +  evenness*size  + log(dilfactor)", data = csv, family = my_family))
  # 168.29 (mucho complicar quiza)
  
  summary(glm("success ~ size * log(dilfactor) * evenness", data = csv, family = my_family))
  ...
  # evenness                      4.173e+00  7.468e-01   5.588 2.64e-08 ***
  # size:log(dilfactor)           6.135e-07  1.684e-07   3.643 0.000277 ***
  # size:evenness                -1.123e-06  1.184e-06  -0.949 0.342923    
  # log(dilfactor):evenness       4.317e-01  1.160e-01   3.720 0.000205 ***
  # size:log(dilfactor):evenness -4.071e-07  1.801e-07  -2.261 0.023887 * 
  # just choose mymodel
  ...
  # assess model --------------------------------------------------------------
  myformula <- "success ~ log(dilfactor):evenness + evenness  + size  + log(dilfactor)"
  mymodel <- glm(myformula, data = csv, family = my_family)
  summary(mymodel)
  # Deviance Residuals: 
  ...
  png(paste0(out_folder, "/", prefix, "dharma_simulateResiduals.png"), width = 1000, height = 600)
  ...
  ## mymodel2 <- glm("success ~ evenness  + size  + log(dilfactor)", data = csv, family = my_family)
  ...
  rich_rf2_small_log <- cforest(success~size + logdilfactor + shannon + richness,
                                data=csv)
  
  predictionsL <- compare.fits(formula = success~size + logdilfactor + shannon + richness,
                               data = csv,
                               model1 = rich_rf2_small_log,
                               return.preds = T)
  
  png(paste0(out_folder, "/", prefix, "success_by_dilfactorxsize.png"), width = 1200, height = 500)
  flexplot(formula = success~ shannon | logdilfactor + size,
  ...
  mymodel1 <- glm("success ~ evenness  + log(dilfactor) + richness",
  ...
  mymodel2 <- glm("success ~ evenness  + log(dilfactor) + richness",
  ...
  rf_estimates <- estimates(cforest(success ~ size:log(dilfactor) +  evenness  + size  + log(dilfactor),
                                    controls = cforest_control(ntree = 1000),
                                    data=csv))
  ...
  # display
  png(paste0(out_folder, "/", prefix, "success_FI.png"), width = 600, height = 400)
  print(feature_plot)
  dev.off()
  ...
  # display
  png(paste0(out_folder, "/", prefix, "success_coefs.png"), width = 600, height = 400)
  print(coef_plot)
  dev.off()
}

save.image(out_folder, "/RF_success.RData")