...
# ESTE SCRIPT ES MÁS SIMPLE QUE EL "_PREPARSED" A SECAS, que es el que muestra las barritas opcionalmente
...
for (nichedistvalue in c("EvenGroups", "SkewedGroups")){
  for (richnessvalue in c(100, 1000)){
    for (n_of_groups in c(3, 10)) {
      df_summary_filtered <- df_summary[((df_summary$nichedist == nichedistvalue) &
                                           (df_summary$richness == richnessvalue) &
                                           (df_summary$nicheN == n_of_groups)), ] %>%
        mutate(nicheN = as.factor(nicheN))
      
      # plots[[nichedistvalue]] <- ggplot(df_summary_filtered, aes(x = cumulative_count, y = mean_success, color = as.factor(nicheN))) +
      plots[[nichedistvalue]] <- ggplot(df_summary_filtered, aes(x = cumulative_count, y = mean_success, color = nicheN)) +
        geom_point(size = 1) +
      ...
      ggsave(
        filename = paste0(out_folder,"/0g_extinction_per_group_", nichedistvalue, "_", richnessvalue, "_", n_of_groups, ".png"),
        plot = plots[[nichedistvalue]],
