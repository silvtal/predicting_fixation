...
# read data ===============================================================
csv = pd.read_csv(f"~/predicting_fixation/1_datasets/simulation_results/processed_data_simcomms_groups_{perc}", encoding = "UTF-16")
    ...
    for c in range(num_cols):
        ...
        for i, value in enumerate(values):
            ...
            for dun, du in enumerate(selection.distrib.unique()):
                ...
                axs[r, c].scatter(selection2.index, selection2['success'], c=my_c, facecolors='none', alpha=0.5, marker=marker)
            axs[r, c].set_title(f'{var} = {value}')
            ...
            if plot_legend:
                legend = axs[r, c].legend(handles=legend_handles, labels=unique_labels, loc='best', frameon=False)
