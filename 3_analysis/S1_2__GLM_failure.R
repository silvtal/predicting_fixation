...
results <- list()
out_folder = "../figures_groups/2__GLM_failure/"; if (!file.exists(out_folder)) {system(paste("mkdir -p", out_folder))}

#> only 0.5 for groups scenario
threshold <- 0.5
# OPTIONS -----------------------------------------------------------------
...
# Load the input data
fcsv <- read.csv(my_file, fileEncoding = "UTF-16", sep = ",")
