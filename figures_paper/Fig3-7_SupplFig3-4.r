...
# ==============================================================================
# plots
load(".debug_ALL_information.RData"); output_figure_folder <- "." # debug
...
for (ctype in unique(all_information$community_type)) {
  ...
  if (my_study_var == "comm_number") {
    plot_title <- "Success rate for each community"
  } else {
    plot_title <- paste("Relationship between", var_traduccion[my_study_var], "and success rate")
  }
  grafico <- ggplot(success_rates_ctype, aes(x = !!sym(my_study_var),
    ...
    labs(
      # title = plot_title, # comment out for the paper
      subtitle = paste0(
        sapply(my_subtitle_vars, function(var) {
          nombre_en <- ifelse(!is.na(var_traduccion[var]), var_traduccion[var], var)
          paste0(nombre_en, ": ", unique(success_rates_ctype[[var]]), ". ")
        }),
        collapse = ""
      ),
      x = if (my_study_var == "comm_number") {"Community"} else {my_study_var},
      y = "Success rate",
      color = "Community"
    ) +
    theme_minimal() +
    ...
    theme(
      ...
      strip.text = element_text(size = 15) # for subplot (facet) titles
    )
  
  fig_name <- ctype_to_fig[ctype]
  ggsave(
    filename = paste0(file.path(output_figure_folder, fig_name), ".png"),
    plot = grafico,
    width = 14, height = 10
  )
  # save.image(paste0("../../../3_analysis/debug/", fig_name))
}
