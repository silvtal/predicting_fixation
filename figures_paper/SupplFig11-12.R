...
# FILE OPTIONS ----------------------------------------------------------------
#> different output folder ("_paper")
out_folder = "."
# ---
# no interactions
...
for (INTER_FOLDER in INTER_FOLDERS) {
  ...
  for (dilfactorvalue in c(0.1)) {
    ...
    df_summary_filtered_i <- df_summary_i[((df_summary_i$nichedist == nichedistvalue) &
      ...
      rename(
        "Interaction presence" = "Interaction_presence",
        "Interaction sign" = "Interaction_sign",
    ...
    df_labels <- df_summary_filtered_i %>%
      group_by(`Interaction value`, `Interaction presence`, `Interaction sign`) %>%
    ...
    p <- ggplot(df_summary_filtered_i, aes(
      x = group_number, 
      y = mean_extinction, 
      color = as.factor(`Interaction value`),
      group = `Interaction value`)
    ) +
      ...
      theme_minimal() +
      facet_grid(
        rows = vars(`Interaction presence`),
        cols = vars(`Interaction sign`),
        axis.labels = "all",
        labeller = label_both
      ) +
    ...
    fig_prefix <- ifelse(nicheNvalue == 3, "SupplFig11", "SupplFig12")
    ggsave(
      filename = paste0(out_folder, "/", fig_prefix, "_0g_extinction_per_group_", INTER_FOLDER, "_", dilfactorvalue, ".png"),
      plot = p,
