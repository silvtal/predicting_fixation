"""
This script generates functional groups tables in batch. It consists of a loop that calls the `generate_table` function multiple
...
Simulations (the next step):
    Each community (abundance table; see "simcomms" folder) will be paired with those groups tables with the same number of
    species. This means multiple groups tables per community. Each community will then be simulated with a varying number of
 ...
 - No interactions yet.
      > Adding interactions means using a different script to create interaction tables. After that,
        we would need to make simulations for all combinations of:
   	  ...
   	  - Groups tables  (with the same LN as the communities)
...
from generate_groups_table import generate_groups_table
...
output_folder = 'functional_groups_tables'
...
for LN in LN_values:
    for N in N_values[LN]:
        for distr in ['Even', 'Skewed']:
          ...
          generate_groups_table(N, Av, LN=LN, outfile=output_folder + '/' + outfile)
